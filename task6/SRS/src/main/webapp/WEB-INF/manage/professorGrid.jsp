...
<script type="text/javascript">
	$(function(){
		//
		$('#dg').datagrid({
			url:'${basePath}/professor_query.action',
			//width:200,
			frozenColumns:[[
			...
			toolbar:[{
				...
				handler: function(){
					...
					$("#win").window({
						...
						closable:true,
						modal:true,
						content:"<iframe src='${basePath}/send_manage_saveProfessor.action' scrolling='no' width='100%' height='100%' frameborder='0' />",
					});
				...
				handler: function(){
					...
					if(rows.length!=1){
						...
						$("#win").window({
							...
							closable:true,
							modal:true,
							content:"<iframe src='${basePath}/send_manage_updateProfessor.action' scrolling='no' width='100%' height='100%' frameborder='0' />",
						});	
					}
				}
			},'-',{
						...
						$.messager.confirm('确认对话框','确认要删除选中的行吗?',
										function(r) {
											if (r) {
												...
												$.post("${basePath}/professor_delete.action",
													{ids : ids}, 
													function(result){
		    ...
		    columns:[[
		        {field:'ssn',title:'教师编号',width:200},
		        {field:'department',title:'部门',width:200},
		        {field:'name',title:'姓名',width:200},
// 		        {field:'title',title:'教师',width:200},
		        {field:'title',title:'教师',width:200,align:'right',
